#!/usr/bin/env python3
"""Generate matched SFT and DPO V3 responses for frozen H5 DHE development cases.
...
def load_jsonl(path: Path) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        if row["partition"] != "development":
            raise RuntimeError(f"{row['case_id']}: H5 requires development partition")
...
def main() -> None:
    ...
    parser.add_argument("--training-config", default="configs/training/dpo_v3_e4b_corrective.yaml")
    ...
    cfg = yaml.safe_load(config_path.read_text(encoding="utf-8"))["h5_dhe_v3_pilot"]
    ...
    if cfg["status"] != "frozen_pre_generation" or cfg["partition"] != "development":
        raise RuntimeError("H5 config is not frozen_pre_generation/development")
    ...
    dpo_path = out / "dpo_v3_corrective.jsonl"
...
if __name__ == "__main__":
    try:
        ...
        if out is not None:
            out.mkdir(parents=True, exist_ok=True)
            message = str(exc)
            for sensitive in ("sft_reference", "dpo_v3_corrective"):
