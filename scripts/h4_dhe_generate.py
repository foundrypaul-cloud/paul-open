#!/usr/bin/env python3
"""Generate matched SFT and DPO V2 responses for the frozen H4 DHE pilot.
...
def load_jsonl(path: Path) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        if row["partition"] != "development":
            raise RuntimeError(f"{row['case_id']}: H4 requires development partition")
...
def generate_source(
    ...
    *,
    ...
    expected_scripts = {"hi": "Devanagari", "bn": "Bengali"}
    start = time.time()
    ...
    return {"records": len(records), "elapsed_seconds": elapsed, "sha256": sha256(output_path), "gates": gate_details}
...
def main() -> None:
    ...
    parser.add_argument("--training-config", default="configs/training/dpo_v2_e4b_corrective.yaml")
    ...
    cfg = yaml.safe_load(config_path.read_text(encoding="utf-8"))["h4_dhe_pilot"]
    ...
    if cfg["status"] != "frozen_pre_generation" or cfg["partition"] != "development":
        raise RuntimeError("H4 config is not frozen_pre_generation/development")
    ...
    (out / "generation_manifest.json").write_text(json.dumps(manifest, indent=2, ensure_ascii=False, sort_keys=True) + "\n", encoding="utf-8")
